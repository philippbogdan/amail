...
- Every send uses targeted Accessibility operations on amail's own compose window to avoid Mail's scripted-body quoting bug. Replies and forwards come from Mail's `reply` and `forward` verbs with the new text inserted above Mail's history; amail never writes quote markers or attribution text. Keep composition serialised, preserve the clipboard, and verify the saved draft before submission. Never extract Apple credentials or add automatic account switching after a send restriction.
