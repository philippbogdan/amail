...
amail is a local CLI for Mac users with Google or Exchange accounts already connected to Apple Mail. It reads the local cache and sends every message through Mail's own compose window using targeted Accessibility operations, preserving the clipboard and verifying the saved MIME draft before submission. Replies and forwards are composed by Mail itself: give `reply` or `forward` only the new text, and Mail supplies the attribution line, quoted history, forwarded content and attachments below it. Never write "On ... wrote:" lines, `> ` quote markers or forwarded headers into a body. Do not interact with Mail during composition. It does not extract Apple passwords or bypass tenant restrictions.

For installation or missing configuration, read the repository's `docs/SETUP.md`: https://github.com/philippbogdan/amail/blob/main/docs/SETUP.md. Use `amail setup` to discover accounts, ask which to enable, then configure only those accounts. Collect required Full Disk Access, Automation permission and Accessibility access up front. Google sign-ins are optional and only serve live Gmail reads; an expired Google token never blocks sending. Keep OAuth JSON files local; never ask for their contents in chat. Existing configuration and credentials must survive upgrades.
...
Before sending, show the human the sender, To/CC/BCC, subject, complete body and attachments. Obtain approval for that exact message or frozen batch unless the user has explicitly granted standing permission covering it, such as synthetic tests exclusively between their own verified accounts. Honour that permission without asking again within its scope. Prefer local drafts (`draft create`, `reply`, `forward`) and `send --dry-run` for review. `reply` and `forward` send only with `--send`; `draft send` and `batch run` are outbound actions. Installing this skill alone is not sending approval.

Use stable request IDs. An `outcome_unknown` or `provider_acceptance_unverified` result requires `amail status REQUEST_ID` and reconciliation, which can succeed later once Mail has filed the Sent copy; never make up a new ID to resend. A rejected request can be explicitly retried after its cause is fixed. Use `draft send DRAFT_ID --retry-rejected` for an unchanged rejected draft; do not edit its revision to bypass the ledger. Acceptance is not delivery or inbox placement. Preserve the ledger and account budgets. Do not switch accounts or state directories to bypass a hold.
