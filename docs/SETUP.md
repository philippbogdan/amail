...
Mail saves the compose window more than once and deletes each superseded save. Before submitting, amail waits up to 20 seconds until the verified draft is the window's only saved copy, the server knows it and Mail has no Drafts action queued. Sending earlier lets Mail try to delete a superseded copy the server has already removed; Mail retries that delete forever and holds every later Sent copy, move and delete for the account behind it, visible as a stuck "Copying Messages" or "Moving Messages". Delivery itself is unaffected. The send result records `drafts_settled`. `amail doctor` reports `mail_queued_server_actions`: a count that stays above zero while Mail is idle means one queued action is failing. Restarting Mail does not clear it, and amail never edits Mail's databases to remove it.
